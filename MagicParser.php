<?php $MagicParser_copyright = "Copyright (c) 2005-2011 IAAI Software, All Rights Reserved";
    ...
    while (!$done) {
        ...
        if ($char === false) {
            ...
            $commit = true;
                ...
                case "\r": if ($MagicParser_csv_eol_ignorecr) {
                        break;
                    } case "\n": if (($position > 1) && !$inquote) {
                        $commit = true;
                        $done = true;
                    } break;
                case $MagicParser_csv_field_separator: if (!$inquote) {
                        $commit = true;
                    } else {
                        $usechar = true;
                    } break;
                case $MagicParser_csv_text_delimiter: if ($MagicParser_csv_text_delimiter != chr(0)) {
                        $inquote = !$inquote;
                    } break;
